...
module.exports = function(settings) {
    return async function(runner, screens) {
        if (screens) await runner.screenshot({path: path.join(settings.tests.screenshots, `${settings.tests.step(runner)}wizard-welcome.png`)})

        await runner.click('.test-wizard-welcome-next')

        await runner.waitFor('.component-wizard-telemetry')
        if (screens) await runner.screenshot({path: path.join(settings.tests.screenshots, `${settings.tests.step(runner)}wizard-telemetry.png`)})
        ...
        if (settings.modules.builtin.user.adapter === 'vjs-adapter-user-vg') {
            ...
            if (screens) await runner.screenshot({path: path.join(settings.tests.screenshots, `${settings.tests.step(runner)}wizard-account.png`)})
            await runner.click('.test-wizard-account-next')
        }

        await runner.waitFor('.component-wizard-mic-permission')
        if (screens) await runner.screenshot({path: path.join(settings.tests.screenshots, `${settings.tests.step(runner)}wizard-mic-permission.png`)})
        ...
        if (screens) await runner.screenshot({path: path.join(settings.tests.screenshots, `${settings.tests.step(runner)}wizard-devices.png`)})
        await runner.click('.test-wizard-devices-next')
        // Close audio settings check notification.
        // await runner.waitFor('.component-main-statusbar')
